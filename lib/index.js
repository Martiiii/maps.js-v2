...
maps.GMap = class GMap {
	...
	editArea = (area) => {
		...
		if (feature) {
			this._map.data.overrideStyle(feature, { editable: true });
			oldGeometry = feature.getGeometry();
		}

		this.createUIButton(doneBtnDiv, doneFunction, doneBtnOptions);
		this.createUIButton(cancelBtnDiv, cancelFunction, cancelBtnOptions);
		doneBtnDiv.index = 1;
		cancelBtnDiv.index = 1;
		this._map.controls[google.maps.ControlPosition.TOP_CENTER].push(doneBtnDiv, cancelBtnDiv);
	};
	...
	stopEditArea = (area) => {
		...
		if (feature) {
			this._map.data.overrideStyle(feature, { editable: false });
		}
	};

	setMetadata = (area, properties) => {
		if (typeof area === 'string' || typeof area === 'number') {
			area = this._map.data.getFeatureById(`${area}`);
		} else if (Array.isArray(area)) {
			area.forEach((elem) => {
				if (typeof elem === 'string' || typeof elem === 'number') {
					elem = this._map.data.getFeatureById(`${elem}`);
				}
				this.setMetadata(elem, properties);
			});
		}

		if (area instanceof google.maps.Data.Feature) {
			for (const property of Object.keys(properties)) {
				area.setProperty(property, properties[property]);
			}
			this.saveAreas();
		} else {
			console.log('Given data is not in right type.');
		}
	};
};
